...
export default async function Page({ params }: { params: Params }) {
  ...
  return (
    <article className="box-shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05)] mb-7.5 overflow-hidden rounded-lg border bg-transparent">
      <div className="p-6">
        ...
        {sortedKeys.map((ym) => {
          ...
          return (
            <section key={ym} className="mt-8 border-b">
              <h3 className="mb-3 text-xl text-(--header-color)">
                {year}年{month}月
              </h3>
              <ul className="m-0 flex list-none flex-wrap gap-x-3 gap-y-2 p-0">
                {groups[ym].map((post) => (
                  <li className="mx-0 my-1" key={post.slug}>
                    <Link
                      className="rounded-none bg-transparent p-0 text-lg font-normal text-(--header-color) no-underline hover:underline"
                      href={`/post/${post.slug}`}
                    >
                      {post.title}
                    </Link>
